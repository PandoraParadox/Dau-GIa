...
function AdminProduct() {
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`http://localhost:8081/api/v1/products`)
      .then((res) => {
  ...
  const confirmChange = (id) => {
    axiosInstance.delete(`http://localhost:8081/api/v1/products/${id}`)
      .then(() => {
  ...
  const handleSearch = async (e) => {
    ...
    const response = await axiosInstance.get(url + `products/by-search?page=1&size=1000000&search=${search}`);
              ...
              <tbody>
                {data.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td>{product.auctionTime.replace("T", " ")}</td>
                    <td>
